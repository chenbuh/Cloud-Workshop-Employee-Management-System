...
### 忽略文件
项目根目录下包含 `.gitignore` 文件，已配置忽略以下内容：
...
### 冲突处理
如果在 `git pull` 或 `git merge` 时遇到冲突：
1. **识别冲突**：冲突文件中会出现 `<<<<<<< HEAD` 标记。
2. **手动解决**：根据业务逻辑选择保留的代码。在处理 API 或 View 文件时，通常应保留最新的功能扩展。
3. **标记解决**：冲突解决后，使用 `git add <file>` 标记。
4. **完成提交**：所有冲突解决后，执行 `git commit`。

**注意**：请勿将编译后的 `.class` 文件或 `target/` 目录提交到 Git 仓库。如果不慎提交，请使用 `git rm --cached -r backend/target` 进行清理。
